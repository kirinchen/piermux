...
- D-25(2026-06-18,owner 回報 Android 兩個輸入手感 bug):
...
- D-41(2026-09-01,Tide #137 第一階段「拜師學藝」):**字寬殘字 bug 家族(D-28→D-37)根治研究 —— 只產研究筆記 + refactor 提案,沒動任何 `src/` / `src-tauri/` 檔**。完整筆記:[`doc/note/herdr-control-mode-study.md`](doc/note/herdr-control-mode-study.md)。
  ...
  - **Phase 0 實測結果(2026-09-01,Kirin 拍板後執行)—— D-34 的診斷不成立,b+ 不做**:對本機 tmux 3.4 量 369 個代表性字元(D-34 殘字案例 / CJK / ambiguous / emoji 含 VS16+ZWJ / braille / 半圓 / 框線 / powerline PUA),兩支獨立探針(外部 `#{cursor_x}` 與 pane 內 DSR `\033[6n`)結果 369/369 一致;xterm 側照 `src/lib/xterm-unicode.ts` 的實際組態量。**diff 只有 2 筆**(`ｶ゙` U+FF76+U+3099 tmux=1/xterm=2、ZWSP U+200B tmux=0/xterm=1),**D-34 殘字案例字元 `U R L 驗 證 去 看` 全部一致、零命中**。靈敏度對照證明量得出來:同一管線對 xterm 預設 provider `6`(D-28 之前)抓到 48 筆、`15` 抓到 21 筆。→ **D-28 的 graphemes addon 已經把 tmux↔xterm 字寬接縫關到 0.5%**,D-34 那句根因是 D-28 之後寫的推論、從沒重量過。**選項 b+(字寬探針 + 自訂 `IUnicodeVersionProvider`)降級,不做** —— 做了只會修掉 `ｶ゙` 和 ZWSP,殘字照出。(成本留帳:探針 369 字遠端 CPU 59-64 ms、1 次 SSH exec,「連線時量一次」綽綽有餘,但量了修不到。)
  - **第二假說 H2 + 下一步**:分岔來源是「**xterm.js 會 reflow、tmux 不會**」+ 尺寸/捲動路徑,不是字寬表 —— tmux 增量重繪假設 client grid 與自己逐格相同,只要 reflow 過一次就永久錯位。這同時解釋 F5 必治(D-34/D-37)、拖視窗就好(D-29)、滾輪後才出現(D-33/D-38);字寬假說只解釋得了第一個。**下一步是 Phase 0.5「位元組流重放比對」**(半天,不碰 control mode、不動 `src/`):把 normal attach client 收到的 bytes 全存檔 → 餵給 headless xterm.js(同組態同尺寸)dump grid → 跟 `capture-pane` 的 tmux grid 比對;一致代表殘字產生在 live client 端(修前端就好),不一致才輪到 a2。加碼:重放中途插一次 `term.resize()` 看是否從那刻才分岔 → 直接驗 H2。
  - **Phase 0.6(2026-09-02,Windows 機;tmux 不在手邊,Phase 0.5 真 tmux 重放留給 Linux)—— H2 砍掉兩隻腳 + F5 變蒐證器**:
