...
- **M1g line buffer 開工(本 commit,⭐ 核心賣點)**:`LineBufferInput.tsx` 新元件(原生 `<textarea>` + IME `isComposing` 護欄)+ SessionPanel `inputMode: 'line' | 'stream'` 切換 toggle,**預設 Line**(SPEC §3.5.1)。走 SPEC §9.1 結尾段「server output 唯讀區 + 下方獨立輸入框」fallback 設計(SPEC 原文認可「對 Claude Code 場景更直觀」),跳過 §7.3 onData hold 字元的 race / IME 坑。Send = buffer + `\r`(PTY Enter)。`tsc --noEmit` + `npm run build` 過(1893 modules / 214KB gzip)。**等 owner 驗 acceptance 完成標準:attach Claude Code session → 中文 line mode 打一段 + Enter → Claude 收到完整訊息 + 回覆**。
