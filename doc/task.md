...
- **M2a 完整 ✓(2026-05-13,NOTES D-15)**:Android Studio + NDK r27d (27.3.13750724) + JDK 21 + `.cargo/config.toml` + `gen/android/` scaffold ✓,`cargo check --target aarch64-linux-android` 1m01s 全鏈路過 ✓,**首次實機 boot ✓**(Galaxy `R5CW60V57KH`,`npm run tauri android dev` → APK install + WebView 渲染 React UI)。Gotcha:Tauri 2 預設 `TAURI_DEV_HOST=LAN_IP`,手機必須跟 PC 同 WiFi 子網段;USB-only / 跨網段場景之後需要 `adb reverse` fallback(暫不急,真遇到再做)
- **[EPIC-002](Epic/EPIC-002.md) Android port(M2b..M2e)** — `active` 2026-05-14。對齊 SPEC §8。
  全部範圍由單張 [ISSUE-010](Issue/ISSUE-010.md) 帶頭推進(owner 拍板「不要開太細」),
  撞到重大轉折再 spawn sub-issue。M2b → M2c → M2d ⭐(line buffer × Android IME 核心驗收點)→ M2e(signed release + 解 NOTES D-4)
