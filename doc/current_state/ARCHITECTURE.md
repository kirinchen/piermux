...
- `desktop/HostTree.tsx` — 左側 host/session tree。每 host row 含 `[checkbox]` (multi-select) + chevron + status icon + name + hover [🔄 / ✏ / 🗑]。展開後第一個 child 是 ⚡ shell synthetic row,然後是 tmux sessions。**多 tmux socket(`tmux -L <name>`)時 session 依 socket 分組,每組上方一個不可收合 `-L <socket>` 小標題(D-39,B 方案 `lib/session-group.ts`);只有一個 socket(通常 default)則扁平不顯示標題。** 每個 tmux session row hover 出 [🔄 / ✏ rename / 🗑 kill](走 `useKillSession` / `useRenameSession`,SPEC §6.6)
...
- `SessionScreen.tsx` — `mode: 'capture' | 'attach'` 切。`target.kind === 'shell'` 強制 attach(shell 無 capture)
  - **Capture mode**(M2c)— xterm readonly(font 13、scrollback 5000)+ `captureSession` on mount + `capture-updated:<host>:<socket>:<session>` listen + 右上 [🔄] per-session refresh + `QuickKeyBar` + 一行文字 input + Send(send_message literal=true, send_enter=true)
...
- `hosts.rs` — `Host` / `HostForm` struct + `Session`(含 `socket` 欄,D-39)+ `HostConnectionStatus` + sqlx pool 開啟 + apply_schema + `migrate_capture_cache_socket`(既有 DB 的 capture_cache 加 socket 進 PK 的 idempotent 重建)+ CRUD
- `sessions.rs` — `list_sessions` / `host_status` / `kill_session` / `rename_session` / `new_session`(SPEC §6.6 + session-level rename UX;kill/rename/new 都多收 `socket` 參數,D-39)Tauri commands + 共用 helpers `read_password_for` / `build_auth` / `port_u16` / `parse_sessions` / `list_sessions_for` + `tmux_with_socket`(pub(crate),組 `tmux -L <socket>` 前綴給各 module 共用)+ 內部 `run_tmux_control` / `shell_quote`。**`TMUX_LIST_FMT` 改成一條 shell 掃 `${TMUX_TMPDIR:-/tmp/tmux-<uid>}/` 下每個 socket 各跑一次 `tmux -L <s> list-sessions`(行首帶 socket,5 欄),死 socket 被 `2>/dev/null` 略過 — 只多 parse 不多 SSH round-trip(D-39)**
- `capture.rs` — `capture_session` / `capture_host` / `capture_all`(M1d 三層 refresh;`capture_session` 多收 `socket`,D-39)。`capture_host_inner` 一個 host 一條 SSH 跑多 channel(`Semaphore(3)`,SPEC §9.2),每個 session 用其 `socket` 組 `tmux -L`,emit `capture-updated:<host_id>:<socket>:<session_name>` event,UPSERT `capture_cache`(PK = host_id+socket+session_name)
- `attach.rs` — `attach_session`(多收 `socket`)/ `attach_shell` / `write_to_session` / `resize_session` / `detach_session` / `scroll_session`(M1f + D-14 + D-24 + D-39)。`AttachRegistry: Mutex<HashMap<String, AttachHandle>>` 存 attach 狀態(含 `target: Option<(String, String)>` = (socket, session) 供 copy-mode 用),reader task 把 PTY 輸出 emit `attach-output-<id>`,結束 emit `attach-closed-<id>`。`scroll_session`(D-24)在同一條 SSH 連線加開 exec channel 跑 `tmux -L <socket> copy-mode` + `send-keys -X scroll-up/down`,讓 alt-screen 滾輪能看歷史而不碰 PTY stdin;shell target(無 tmux)no-op
- `messaging.rs` — `send_message(host, socket, session, payload, send_enter, literal)`(M1e + D-12 + D-39),走 `tmux -L <socket> send-keys` literal 或 named-key
- `upload.rs` — `upload_to_session(host, socket, session, local_path)`(D-40)。先 `tmux -L <socket> display-message -p -t <session>:0 '#{pane_current_path}'` 拿該 pane 活的 cwd,組 `<pwd>/<basename>` 上傳(覆蓋同名),回完整遠端路徑給前端 toast。後端自己 `std::fs::read` local_path(desktop 拖放給路徑,IPC 不扛 bytes;512MB 上限防呆)。只 tmux target(shell 無活 cwd)
...
- `ssh.rs` — makiko 0.2.5 wrapper:`connect()` 回 `SshSession`(共用 connection)+ `SshSession::exec()` + `run_command()`(one-shot)+ `test_connection()` + `SshSession::upload()`(D-40,exec `cat > <path>` + 串流 `send_stdin`/`send_eof`,makiko 沒 SFTP)。Server pubkey 接受 any(M1b 起);auth 支援 password + key(Ed25519/RSA);`SshSession::client()` 暴露給 attach.rs 開 PTY channel
- `migrations/0001_initial.sql` — 4 張表:hosts / ui_preferences / quick_presets / capture_cache(SPEC §5;capture_cache PK 含 `socket`,D-39,既有 DB 由 `hosts.rs::migrate_capture_cache_socket` 升級)
...
4. 每個成功 capture:UPSERT `capture_cache` 表 + `app.emit("capture-updated:<host>:<socket>:<session>", &result)`(D-39)
...
*Last updated: 2026-08-28(D-40:拖放上傳檔案到 remote pane current pwd —— `upload.rs` command 先 `display-message '#{pane_current_path}'` 拿活的 cwd 再 `ssh.rs::upload`(exec `cat >`+串流 stdin,makiko 沒 SFTP)寫進去;desktop `SessionPanel` tmux target 掛 `getCurrentWebview().onDragDropEvent`,拖檔→上傳→toast 回遠端路徑,只 tmux(shell 無活 cwd);後端讀本地路徑(IPC 不扛 bytes),同名覆蓋,Android 第二刀。以下為先前紀錄:D-39:tmux socket(`-L`)分層 —— `list_sessions` 改一條 shell 掃 `${TMUX_TMPDIR:-/tmp/tmux-<uid>}/` 下所有 socket 各跑 list-sessions(5 欄含 socket)、`Session`/`CaptureResult` 加 `socket` 欄、kill/rename/new/attach/capture/send/scroll 全走 `tmux -L <socket>`、`capture_cache` PK 加 socket(既有 DB idempotent 重建)、capture event 名 `capture-updated:<host>:<socket>:<session>`、tree/list 依 socket 分組(B 方案,只有一個 socket 時扁平,`lib/session-group.ts` desktop+Android 共用);host 層 [+] 新 session 一律建 default socket。以下為先前紀錄:D-35:終端字型 / 字級可設定 —— `lib/term-prefs.ts` + `lib/useTermPrefs.ts`(localStorage,非 `ui_preferences` 表)、desktop `SettingsDialog` + Android `SettingsScreen`,四個 xterm 都讀,改動即時生效不 remount。D-36:終端網址點一下開 OS 預設瀏覽器 —— `lib/xterm-links.ts` 走 `tauri-plugin-opener`,四個 xterm 都掛,只放行 http/https(capability scope 再擋一層)。以下為先前紀錄:D-30:attach 後主動 nudge 尺寸(rows-1→rows)逼 tmux 乾淨全重畫 — 修「非全寬/直式 attach 花屏殘留」,實測尺寸其實一致(window-size=latest 單 client),根因是首次繪製撞上 xterm reflow、送同尺寸 resize 不會重畫,故 nudge 一次真正尺寸變化;header 加 `cols×rows` 診斷顯示。D-29(前一版方向對機制錯、無效,已被 D-30 取代)。D-28:`lib/xterm-unicode.ts` — 加 `@xterm/addon-unicode-graphemes` 對齊 emoji/CJK 寬度(真字寬 bug,但非 D-29 那個行頭殘留主因);OSC 52 解析經真實 tmux bytes 證明乾淨。D-27:CTRL/ALT 改 toggle(hold);attach 逐鍵輸入改 `inputmode="url"`。D-26:`useTouchScroll` 手指拖曳捲動。D-25:bar 容器層 `onMouseDown.preventDefault` 保軟鍵盤焦點)*
